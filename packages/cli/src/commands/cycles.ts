...
import { outputJson, formatCyclesTable, outputTable } from "../output.js";
import { resolveProjectId } from "../helpers.js";

export function registerCyclesCommands(parent: Command): void {
  const cycles = parent.command("cycles").description("Manage cycles/sprints");
  ...
  cycles
    ...
    .action(async (cmdOpts) => {
      ...
      try {
        ...
        const cycleList = await client.getCycles(projectId);

        if (opts.json) {
          outputJson(cycleList);
        } else {
          if (cycleList.length === 0) {
            ...
            outputTable(
              formatCyclesTable(
                cycleList as unknown as Array<Record<string, unknown>>,
              ),
            );
          }
        }
